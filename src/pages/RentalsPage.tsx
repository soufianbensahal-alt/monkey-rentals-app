...
export default function RentalsPage() {
  ...
  return <div className="fade-up">
    <PageHeader eyebrow="Contratos" title="Alquileres" description="Gestiona alquileres por días, semanas y meses con tarifas automáticas editables." action={<button className="btn-primary" disabled={!canCreate} onClick={() => open(blank())}><Plus size={18}/> Crear alquiler</button>}/>
    ...
    <div className="table-shell">{rows.length ? <table className="data-table"><thead><tr><th>Alquiler</th><th>Fechas</th><th>Estado</th><th>Próximo pago</th><th>Precio</th><th>Km previstos</th><th>Acciones</th></tr></thead><tbody>{rows.map(rental => {
      const vehicle = vehicleById.get(rental.vehicleId)
      const customer = customerById.get(rental.customerId)
    ...
    })}</tbody></table> : <EmptyState title="No hay alquileres creados." description={canCreate ? 'Crea el primer alquiler y elige su periodo.' : 'Añade primero un vehículo y un cliente.'} action={canCreate ? <button className="btn-primary" onClick={() => open(blank())}><Plus size={18}/> Crear alquiler</button> : undefined}/>}</div>
