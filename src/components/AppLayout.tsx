    ...
    <div className="min-w-0 flex-1">
      <header className="sticky top-0 z-20 flex min-h-18 items-center gap-3 border-b border-orange-100 bg-white/95 px-4 backdrop-blur-xl sm:px-6 md:min-h-20 md:px-8">
        ...
        <div className="relative hidden max-w-sm flex-1 md:block">
          <label className="relative block"><span className="sr-only">Buscar en toda la app</span><input className="field search-field bg-brand-50/40" value={search} onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)} onChange={event => setSearch(event.target.value)} placeholder="Buscar vehículo, cliente o alquiler..."/>{!searchFocused && !search && <Search className="search-icon pointer-events-none absolute top-1/2 -translate-y-1/2 text-brand-500" size={18}/>}</label>
